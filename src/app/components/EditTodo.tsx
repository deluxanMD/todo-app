    ...
    return (
       <div className='flex flex-col items-center justify-center'>
            <div className='mt-20 text-center w-100'>
                <h2 className='h2 uppercase font-extrabold text-4xl w-full'>Edit Todo</h2>
                <div className='mt-10'>
                    <input className="border-1 rounded p-2 w-100" value={value?.title} onChange={handleChange} />
                    {!!todo && todo?.dependsOn?.length > 0 && (
                        <div className="mt-5 text-left">
                            <h2 className="font-bold">Dependent Tasks</h2>
                            {todo?.dependsOn?.map(depId => (
                                <div key={depId} className="flex flex-row items-center space-x-4">
            ...
            <div className="mt-5">
                <button className="w-100 bg-blue-400 p-2 rounded text-white cursor-pointer hover:bg-blue-500">Update</button>
            </div>
        </div>
    )
}